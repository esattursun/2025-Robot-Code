...
import frc.robot.Constants.TELEOP_AUTO_ALIGN;
...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  //  private static final Matrix<N3,N1> localMeasurement = VecBuilder.fill(0.05, 0.05, Units.degreesToRadians(5));
  //  private static final Matrix<N3,N1> visionMeasurement = VecBuilder.fill(0.05, 0.05, Units.degreesToRadians(30));

  //  public final SwerveDrivePoseEstimator swerveDrivePoseEstimator; // x,y,heading in radians; Vision measurement std dev, higher=less weight
...
//    swerveDrive.pushOffsetsToEncoders(); // Set the absolute encoder to be used over the internal encoder and push the offsets onto it. Throws warning if not possible
  ...
  public void periodic(){
    if (visionDriveTest){
      ...
      // vision.updateSwerveDrivePoseEstimation(swerveDrivePoseEstimator);
...
PathConstraints constraints = new PathConstraints(
  getSwerveDrive().getMaximumChassisVelocity()-4,
  getSwerveDrive().getMaximumChassisAngularVelocity()-4,
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public double RobotToCoralStationPoseright(){

    Pose2d coralstationpose =gettrueCoralStationPose(0.7, 0.16);
    Pose2d robotPose = getPose();

    Transform2d Error =coralstationpose.minus(robotPose);

    return Math.abs(Error.getX())+Math.abs(Error.getY());
  }
 ...
 */
  public Pose2d gettrueCoralStationPose(double vertical,double horizontal ){
    ...
    public Pose2d gettrueCoralStationPosenet(double vertical,double horizontal ){
      Station station = getclosestCoralStation();
...
public ChassisSpeeds getAlignmentSpeeds(Pose2d desiredPose) {
  ChassisSpeeds speeds =TELEOP_AUTO_ALIGN.TELEOP_AUTO_ALIGN_CONTROLLER.calculate(getPose(), desiredPose, 0,
  ...
  // Pose2d coralStationpose =new Pose2d(
        //   station.getStationX()-xchangefactor(station,vertical,horizontal),
        //   station.getStationY()-ychangefactor(station, vertical,horizontal),
        //   new Rotation2d(Math.toRadians(getrobotshoulddegree())));

  // public double RobotToCoralStationPoseleft(){
  //   Pose2d coralstationpose =gettrueCoralStationPose(0.7, -0.18);
  //   Pose2d robotPose = getPose();

  //   Pose2d Error =coralstationpose.relativeTo(new Pose2d(robotPose.getTranslation(),Rotation2d.fromDegrees(getclosestCoralStation().getRotationCoralStation())));
  //   return Math.abs(Error.getX())+Math.abs(Error.getY());
  // }
  // public Pose2d gettrueCoralStationPose2(double vertical,double horizontal){
  //   Pose2d coralPose2d = new Pose2d(
  //     getclosestCoralStation().getStationX(),getclosestCoralStation().getStationY(),Rotation2d.fromDegrees(getclosestCoralStation().getRotationCoralStation()
  //   ));
  //   coralPose2d.relativeTo();
  //   return null;
  // }
...
//   public static double find60degreeMeasurement(double x) {
//     return (x / 2) * Math.sqrt(3);
// }

// public static double find30degreeMeasurement(double x) {
//   return x / 2;  
// }
      ...
      //   station.getStationX()-xchangefactor(station,vertical,horizontal),
      //   station.getStationY()-ychangefactor(station, vertical,horizontal),
      //   new Rotation2d(Math.toRadians(station.getRotationCoralStation())));


//       private double xchangefactor(Station station,double distance,double ydistance) {
//         double xchangefactor;
//         switch (station) {
//           case AB:
//               xchangefactor = distance;
//               break;
//           case CD:
//                 xchangefactor= find30degreeMeasurement(distance)-find60degreeMeasurement(ydistance);
//                 break;
//           case EF:
//                 xchangefactor= -find30degreeMeasurement(distance)-find60degreeMeasurement(ydistance);
//                 break;
...
//         private double ychangefactor(Station station,double distance,double ydistance) {
//         double ychangefactor;
//           switch (station) {
//             case AB:
//                 ychangefactor= 0+ydistance;
//                 break;
//             case CD:
//                 ychangefactor= find60degreeMeasurement(distance)+find30degreeMeasurement(ydistance);
//                 break;
//             case EF:
//                 ychangefactor= find60degreeMeasurement(distance)-find30degreeMeasurement(ydistance);
//                 break;
...
//           private double getrobotshoulddegree(){
//             double robotshould = (getclosestCoralStation().getRotationCoralStation());
//             return robotshould;
//            }
